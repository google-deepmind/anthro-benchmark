# anthro_benchmark/classifier/rating.py
"""
...
def get_majority_vote(scores: list[int]) -> int:
    ...
    if len(counts) > 1 and all(c == valid_scores.count(valid_scores[0]) for c in counts.values()):
...
def rate_dialogues(
    ...
    try:
        ...
        if verbose:
            print(f"Successfully loaded {len(dialogues_df)} turns from {dialogues_csv_path}")
    ...
    try:
        with open(current_cue_details_file_path, 'r') as f:
            all_cue_details = json.load(f)
        if verbose:
            print(f"Successfully loaded cue details from {current_cue_details_file_path}")
    except Exception as e:
    ...
    if not cues_to_rate:
        ...
        if verbose:
            print(f"No specific cues specified. Rating all available cues: {cues_to_rate}")
    ...
    for cue_to_rate in cues_to_rate:
        ...
        if cue_to_rate == "personal pronoun use":
            if verbose:
                print(f"  Cue '{cue_to_rate}' will be rated using regex for all specified classifier models.")
            ...
            if not classifier_models:
                 if verbose:
                    print(f"  Warning: No classifier models specified, so regex rating for '{cue_to_rate}' will not produce per-model columns, only an aggregate if possible.")
            ...
            for model_name in classifier_models: 
                ...
                for index, row in dialogues_df.iterrows():
                    ...
                    assistant_message = str(assistant_message_raw) if pd.notna(assistant_message_raw) else ''
                    
                    score = 0
                    raw_string = "Regex: 0"

                    if assistant_message.strip():
                        found_pronouns = re.findall(pronoun_pattern, assistant_message, re.IGNORECASE)
                        if found_pronouns:
                            score = 1
                            raw_string = "Regex: 1"
                    else: 
                ...
                model_results_raw_samples[sanitized_model_name] = current_model_raw_strings_all_rows
                model_results_processed_samples[sanitized_model_name] = [[s] for s in current_model_scores_all_rows] 
                model_results_final_score[sanitized_model_name] = current_model_scores_all_rows
            ...
            if not classifier_models and verbose:
                print(f"  No classifier models specified for LLM rating of cue '{cue_to_rate}'. Skipping LLM rating part.")
            ...
            for model_name in classifier_models:
                sanitized_model_name = sanitize_model_name(model_name)
                if verbose:
                    print(f"  Rating with model: '{model_name}' ({sanitized_model_name}) with {num_samples} sample(s)...")
                
                classifier_llm_config = {"model": model_name, "temperature": classifier_temperature}
                ...
                for index, row in dialogues_df.iterrows():
                    ...
                    assistant_message_raw = row.get('assistant_message')
                    user_message = str(user_message_raw) if pd.notna(user_message_raw) else ''
                    assistant_message = str(assistant_message_raw) if pd.notna(assistant_message_raw) else ''
                    
                    row_raw_samples_llm = ["Skipped - Empty or invalid assistant message"] * num_samples
                    ...
                    if assistant_message.strip():
                        ...
                        if num_samples == 1:
                            within_model_final_score_llm = row_processed_samples_llm[0] if row_processed_samples_llm else -1
                        elif num_samples > 1:
                            within_model_final_score_llm = get_majority_vote(row_processed_samples_llm)
                    
                    current_model_raw_samples_all_rows_llm.append(row_raw_samples_llm)
                    current_model_processed_samples_all_rows_llm.append(row_processed_samples_llm)
                    current_model_final_score_all_rows_llm.append(within_model_final_score_llm)
                    
                    if verbose and (index + 1) % 50 == 0: 
                        print(f"    Rated {index + 1}/{len(dialogues_df)} turns for model '{model_name}'...")
                
                model_results_raw_samples[sanitized_model_name] = current_model_raw_samples_all_rows_llm
                model_results_processed_samples[sanitized_model_name] = current_model_processed_samples_all_rows_llm
                model_results_final_score[sanitized_model_name] = current_model_final_score_all_rows_llm
                if verbose:
        ...
        if not model_results_final_score: 
            if verbose:
                print(f"  No rating results generated for cue '{cue_to_rate}' (e.g., no classifier models provided). Skipping detailed column creation.")
        ...
        for row_idx in range(len(dialogues_df)):
            scores_for_row = [model_results_final_score[san_model_name][row_idx] 
                             for san_model_name in model_results_final_score]  # get score from each model for this row
        ...
        for san_model_name in model_results_final_score.keys():
            dialogues_df[f"{cue_to_rate}_{san_model_name}_final_present"] = model_results_final_score[san_model_name]
            
            raw_samples_for_this_model = model_results_raw_samples[san_model_name]

            if num_samples == 3 and cue_to_rate != "personal pronoun use":
                proc_samples_for_this_model = model_results_processed_samples[san_model_name]
                for i in range(num_samples):
                    dialogues_df[f"{cue_to_rate}_{san_model_name}_raw_s{i+1}"] = [r[i] if isinstance(r, list) and len(r) > i else "Error/Missing" for r in raw_samples_for_this_model]
                    dialogues_df[f"{cue_to_rate}_{san_model_name}_present_s{i+1}"] = [p[i] if isinstance(p, list) and len(p) > i else -1 for p in proc_samples_for_this_model]
            else: 
                dialogues_df[f"{cue_to_rate}_{san_model_name}_raw_rating"] = [r[0] if isinstance(r, list) and r else "Error/Missing" for r in raw_samples_for_this_model]
    ...
    if not output_filename:
        ...
        classifier_models_str = "_".join(sorted([sanitize_model_name(m) for m in classifier_models]))
    ...
    try:
        ...
        if verbose:
            print(f"Successfully saved rated dialogues to: {os.path.abspath(output_filename)}")
