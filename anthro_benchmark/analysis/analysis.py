# dialogue_generator/analysis.py
import pandas as pd
...
def load_data(rated_csv_path: str) -> Tuple[pd.DataFrame, Dict[str, List[str]], List[str]]:
    ...
    cue_present_columns = [col for col in df.columns if col.endswith('_present') and '_s' not in col and '_final_present' not in col]
    
    if not cue_present_columns:
        raise ValueError(f"No '*_present' columns found in {rated_csv_path}. Cannot perform analysis.")
    ...
    if not all_mapped_cues.issubset(found_cue_prefixes):
         ...
         print(f"Warning: Cues defined in mapping but not found in CSV's *_present columns: {missing_in_df}")
    if not found_cue_prefixes.issubset(all_mapped_cues):
         missing_in_map = found_cue_prefixes - all_mapped_cues
         print(f"Warning: Cues found in CSV (*_present columns) but not defined in category mapping: {missing_in_map}")

    return df, category_mapping, cue_present_columns

def add_category_counts(df: pd.DataFrame, category_mapping: Dict[str, List[str]], cue_present_columns: List[str]) -> pd.DataFrame:
    ...
    for category, cues_in_category in category_mapping.items():
        ...
        relevant_cols = [f"{cue}_present" for cue in cues_in_category if cue in found_cue_prefixes]
        
        if not relevant_cols:
            print(f"  Skipping category '{category}': No corresponding '*_present' columns found.")
            df_analysis[category_col_name] = 0
            continue
            
        # sum only valid ratings (0 or 1), treat -1 as 0 for the sum
        df_analysis[category_col_name] = df_analysis[relevant_cols].applymap(lambda x: x if x in [0, 1] else 0).sum(axis=1)
        print(f"  Added column: {category_col_name}")
        
    return df_analysis

def calculate_summary_stats(df: pd.DataFrame, category_mapping: Dict[str, List[str]], cue_present_columns: List[str]) -> Dict[str, Any]:
    ...
    for col in cue_present_columns:
        ...
        if len(valid_ratings) == 0:
            ...
            percentage = (valid_ratings.sum() / len(valid_ratings)) * 100 # Sum is count of 1s
        summary["cue_percentages"][cue_name] = round(percentage, 2)
        print(f"  Cue '{cue_name}': {percentage:.2f}% present ({valid_ratings.sum()} / {len(valid_ratings)} valid turns)")
    ...
    for category, cues_in_category in category_mapping.items():
        ...
        if relevant_cols:
             ...
             category_total = df[relevant_cols].applymap(lambda x: 1 if x == 1 else 0).sum().sum()
        summary["category_totals"][category] = int(category_total) # ensure integer count
...
def plot_cue_percentages(cue_percentages: Dict[str, float], output_dir: str):
    ...
    fig = go.Figure([go.Bar(x=cues, y=percentages, text=percentages, textposition='auto')])
    fig.update_layout(
    ...
    try:
        ...
        try:
            ...
            print("  To save PNGs, install kaleido: pip install -U \"kaleido>=0.1.0,<0.2.0\"")
...
def plot_category_radar(category_totals: Dict[str, int], output_dir: str):
    ...
    if not category_totals or len(category_totals) < 3:
        print(f"Skipping radar plot: Need at least 3 categories with totals, found {len(category_totals)}.")
        return
    ...
    fig.update_layout(
        polar=dict(
            radialaxis=dict(
                visible=True,
                range=[0, max(totals) * 1.1 if totals else 1]
            )),
        showlegend=False,
...
def run_analysis(rated_csv_path: str, output_dir: str = "analysis_results", category_mapping_path: str = None):
    """Main function to run the analysis pipeline."""
    print(f"\n--- Starting Analysis ---")
    print(f"Rated CSV: {rated_csv_path}")
    print(f"Output Directory: {output_dir}")
    print(f"Using hardcoded category mapping")
    ...
    try:
        ...
        summary_stats = calculate_summary_stats(df_analysis, category_mapping, cue_present_columns)
